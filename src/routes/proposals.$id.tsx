...
export const Route = createFileRoute("/proposals/$id")({
  ...
  notFoundComponent: () => (
    <div className="max-w-xl mx-auto text-center py-32">
      ...
      <Link to="/explore" className="text-primary mt-4 inline-block">← Back to proposals</Link>
    </div>
  ),
...
function ProposalDetail() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <Link to="/explore" className="text-sm text-muted-foreground hover:text-foreground">← All proposals</Link>
      ...
      <div className="mt-6 mb-10">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className={`text-[10px] uppercase tracking-widest px-2 py-1 rounded border ${
            proposal.status === "ACTIVE" ? "border-emerald-500/30 text-emerald-300 bg-emerald-500/10" :
            proposal.status === "CLOSED" ? "border-zinc-500/30 text-zinc-300 bg-zinc-500/10" :
        ...
        <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
          <div>Creator: <span className="font-mono text-foreground">{shortAddr(proposal.creator)}</span></div>
          <div>Created: <span className="text-foreground">{new Date(proposal.createdAt).toLocaleString()}</span></div>
          {proposal.status === "ACTIVE" && (
            <div>Closes in: <CountdownTimer createdAt={proposal.createdAt} /></div>
          )}
        </div>
      </div>
...
function Row({ color, label, count, pct }: { color: string; label: string; count: number; pct: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="font-mono text-muted-foreground"><span className="text-foreground">{count}</span> · {pct}%</div>
    </div>
  );
}
